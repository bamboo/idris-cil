module IRTS.Cil.MaxStack (maxStackFor, maxStackFor') where

import Language.Cil.Syntax

maxStackFor :: [MethodDecl] -> Int
maxStackFor = maximum . maxStackFor'
...
netStackChange _ (Ldfld   _ _ _ _) = 0
netStackChange _ (Ldflda  _ _ _ _) = 0
netStackChange _ (Ldftn   _ _ _ _ _ _) = 1
-- ldvirtftn?
...
netStackChange _ (Ldsfld  _ _ _ _) = 1 -- docs in language-cil are wrong: it doesn't pop anything, the
                                       -- so-called type ref is part of the instruction operand
netStackChange _ (Ldsflda _ _ _ _) = 1
...
netStackChange _ (Stfld _ _ _ _) = -2
...
netStackChange _ (Stsfld _ _ _ _) = -1
